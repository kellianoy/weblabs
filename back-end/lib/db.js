...
const self = (module.exports = {
  channels: {
    create: async (channel) => {
      ...
      await db.put(
        `channels:${id}`,
        JSON.stringify(merge(channel, { users: [userID.id] }))
      );
      ...
      return new Promise((resolve, reject) => {
        ...
        db.createReadStream({
          ...
          .on("data", ({ key, value }) => {
            channel = JSON.parse(value);
            channel.id = key.split(":")[1];
            if (user.channels.includes(channel.id)) channels.push(channel);
          })
          .on("error", (err) => {
            reject(err);
          })
      ...
      return new Promise((resolve, reject) => {
        ...
        db.createReadStream({
          ...
          .on("data", ({ key, value }) => {
            ...
            user.id = key.split(":")[1];
            if (channel.users.includes(user.id)) users.push(user);
          })
          .on("error", (err) => {
            reject(err);
          })
    ...
    delete: async (id) => {
      ...
      //Let's remove all messages of this channel
      await self.messages.deleteAll(id);
    ...
    join: async (id, joiner) => {
      ...
      if (user.channels.includes(id))
        throw Error("User has already joined this channel.");
  ...
  messages: {
    ...
    get: async (channelId, creation) => {
      if (!creation) throw Error("No creation date given");
      if (!channelId) throw Error("No channel id given");
      const message = JSON.parse(
        await db.get(`messages:${channelId}:${creation}`)
      );
      return message;
    },
    update: async (channelId, creation, newMessage) => {
      if (!creation) throw Error("No creation date given");
      if (!channelId) throw Error("No channel id given");
      const original = JSON.parse(
        await db.get(`messages:${channelId}:${creation}`)
      );
      if (!original) throw Error("message doesn't exist");
      await db.put(
        `messages:${channelId}:${creation}`,
        JSON.stringify(merge(message, newMessage))
      );
      //return the new one
      return merge(original, newMessage);
    },
    ...
    deleteAll: async (channelId) => {
      if (!channelId) throw Error("No channel id given");
      const messages = await self.messages.list(channelId);
  ...
  users: {
    ...
    update: async (email, newUser) => {
      ...
      await db.put(
        `users:${userID.id}`,
        JSON.stringify(merge(original, newUser))
      );
      //return the new one
